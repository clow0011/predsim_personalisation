...
def RunSimulation(case):
    # Import settings.
    from settings import getSettings   
    settings = getSettings()
    if case not in settings.keys():
        raise Exception('No Key: '+case+' in settings.py')
    # %% Settings.    
    ###########################################################################
    # Model settings.
    ...
    nThreads = 16 # default number of threads.
    ...
    momentArmIndices = getMomentArmIndices(leftSideMuscles,
        rightSideMuscles, leftPolynomialJoints, rightPolynomialJoints,
        polynomialData['l'], polynomialData['r'])
    ...
    bounds = bounds(Qs_walk_filt, joints, rightSideMuscles, armJoints, 
                    targetSpeed,pathModelFolder=pathModelFolder)
    ...
    if analyzeResults:
        ...
        for k in range(N_gaitCycle):
            ...
            #print(np.max(
            #        np.abs(hillEquilibriumk_GC.full())))
        ...
        if writeMotionFiles:        
            ...
            # CY : print joint velocities
            data = np.concatenate((tgrid_GC.T, Qds_GC.T), axis = 1)
            labels = ['time'] + joints
            numpy_to_storage(labels,data,os.path.join(pathResults, 'joint_velocities.mot'))
