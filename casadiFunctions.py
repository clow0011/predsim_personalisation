...
def polynomialApproximation(musclesPolynomials, polynomialData, NPolynomial):    
    #CY: musclesPolynomials : left/right/trunk muscles string names
    #CY: polynomialData : dictionary, key:muscle names, values:keys of 'coefficients','dimension','order','spanning'
    from polynomials import polynomials
    import pdb
    # Function variables.
    qin = ca.SX.sym('qin', 1, NPolynomial)
    qdotin  = ca.SX.sym('qdotin', 1, NPolynomial)
    lMT = ca.SX(len(musclesPolynomials), 1) #CY: no of muscles
    vMT = ca.SX(len(musclesPolynomials), 1) #CY: no of muscles
    dM = ca.SX(len(musclesPolynomials), NPolynomial) #CY: no of muscles x number of joints
    ...
    for count, musclePolynomials in enumerate(musclesPolynomials):
        ...
        spanning = polynomialData[musclePolynomials]['spanning'] #CY: no of 1 in array with length number of joints, no of 1 = dimension 
