...
def getMomentArmIndices(leftMuscleNames, rightMuscleNames, leftPolynomialJoints,
                        rightPolynomialJoints, leftPolynomialData, rightPolynomialData):
         
    momentArmIndices = {} #CY: Dictionary with key (joints name) and value (list of muscle index)
    import pdb

    for count, muscle in enumerate(leftMuscleNames): 
        if not muscle in leftPolynomialData:
            print("left muscles: "+muscle+" not found in leftPolynomialData")
            continue
        spanning = leftPolynomialData[muscle]['spanning']
        for i in range(len(spanning)):
            if (spanning[i] == 1):
                momentArmIndices.setdefault(
                        leftPolynomialJoints[i], []).append(count)
    for count, muscle in enumerate(rightMuscleNames):
        if not muscle in rightPolynomialData:
            print("right muscles: "+muscle+" not found in rightPolynomialData")
            continue
        spanning = rightPolynomialData[muscle]['spanning']
        for i in range(len(spanning)):
            if (spanning[i] == 1):
                momentArmIndices.setdefault(
                        rightPolynomialJoints[i], []).append(
                                count + len(leftMuscleNames))                
