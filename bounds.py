...
class bounds:
    ...
    def __init__(self, Qs, joints, muscles, armJoints, targetSpeed, 
                 mtpJoints=['mtp_angle_l', 'mtp_angle_r'],pathModelFolder=None):
        ...
        #CYChange
        self.pathModelFolder = pathModelFolder
        self.BOUNDSSCALINGFACTORINPUT = "BoundsAndScalingFactors.npz"
    ...
    def getBoundsPosition(self):        
        ...
        for count, joint in enumerate(self.joints):  
            if (joint == 'mtp_angle_l') or (joint == 'mtp_angle_r'):
                ...
                # Special cases
                if joint == 'pelvis_tx':
                    upperBoundsPosition[joint] = [2.5] #2
                    lowerBoundsPosition[joint] = [0] 
                elif joint == 'pelvis_ty':
                    upperBoundsPosition[joint] = [1.1]
                    lowerBoundsPosition[joint] = [0.65] #0.75
                elif joint == 'pelvis_tz':
                    upperBoundsPosition[joint] = [0.15] #0.1
                    lowerBoundsPosition[joint] = [-0.15] #-0.1
    ...
    def getBoundsActivation(self):
        #CYChange, change the upper and lower bound of activate to affect the gait
        if self.pathModelFolder == None or not os.path.exists(os.path.join(self.pathModelFolder, self.BOUNDSSCALINGFACTORINPUT)):
            lb = [0.05] 
            ...
            print("use manual bounds and scaling factor for muscle activation")
            p = os.path.join(self.pathModelFolder, self.BOUNDSSCALINGFACTORINPUT)
            data = np.load(p)
            lb_vec = data['Activation_Muscle_lb'].tolist()
            ub_vec = data['Activation_Muscle_ub'].tolist()
            s_vec = data['Activation_Muscle_s'].tolist()
    ...
    def getBoundsFinalTime(self, gaitCycleSimulation='half'):
        if gaitCycleSimulation == 'half':
            upperBoundsFinalTime = pd.DataFrame([1.2], columns=['time']) #1
